...
class WhatsAppService:
    ...
    def _normalize_contacts(self, data: list | dict) -> list:
        ...
        for item in items:
            ...
            if not (item_id.endswith("@c.us") or item_id.endswith("@s.whatsapp.net")):
                continue

            phone_clean = item_id.replace("@c.us", "").replace("@s.whatsapp.net", "")
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    connected = session_status in (
                        "WORKING",
                        "SCAN_QR_CODE",
                        "STARTING",
                    )
                    return {"connected": connected, "status": session_status}
                else:
                    return {
                        "connected": False,
            ...
            logger.error("WhatsApp API connection timeout")
            return {"connected": False, "status": "timeout"}
        except httpx.ConnectError:
            logger.error("WhatsApp API connection refused")
            return {"connected": False, "status": "connection_refused"}
        except Exception as e:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return {
                        ...
                        "chatId": data.get("chatId") or data.get("jid"),
                    }
                else:
