"""
Router para gestión de background jobs
"""
...
@router.get("/tipo/{tipo}/activo")
async def obtener_job_activo(
    tipo: str,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(get_current_user)
):
    """Obtener job activo de un tipo específico"""
...
@router.get("/tipo/{tipo}/ultimo")
async def obtener_ultimo_job(
    tipo: str,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(get_current_user)
):
    """Obtener último job de un tipo específico"""
    job = db.query(BackgroundJob).filter(
        BackgroundJob.tipo == tipo
...
async def cancelar_job(
    ...
    """Cancelar un job (solo si está pendiente)"""
    job = db.query(BackgroundJob).filter(BackgroundJob.id == job_id).first()
